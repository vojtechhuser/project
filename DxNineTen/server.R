...
shinyServer(function(input, output, session) {
  ...
  previousPage <- NULL
  output$dxvalue=renderPrint({input$dx})
  
  output$dxhits <- renderDataTable({
    #code<-str_replace_all(input$dx,'\\.','')
    df<-dplyr::filter(kb,grepl(paste0('^',input$dx),concept_code)) %>% select(-domain_id,-valid_start_date,-concept_class_id)
    table<-datatable(df, options = list(searching = FALSE))
    return(table)
  })
  
  
  output$relhits <- renderDataTable({
    #code<-str_replace_all(input$dx,'\\.','')
    df<-dplyr::filter(relkb,grepl(paste0('^',input$dx),concept_code.x)) 
    table<-datatable(df, options = list(searching = FALSE))
    return(table)
  })
  
  
  output$csshits <- renderDataTable({
    #find the category and display it
    categories<-dcss %>% filter(concept_code==input$dx) %>% select(ccs_ctg) %>% distinct()
    df<-dcss %>% inner_join(categories)
    #df<-dplyr::filter(dcss,concept_code=='01000')
    table<-datatable(df, options = list(searching = FALSE))
    return(table)
  })
